  ...
  const validatePassword = (pw: string): string | null => {
    if (type === 'employer') {
      if (!/^(?=.*[A-Za-z])(?=.*\d).{8,}$/.test(pw)) {
        return 'Min. 8 tekens, met een letter én een cijfer.';
      }
    } else if (pw.length < 8) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        setError(data.message || 'Wachtwoord opnieuw instellen mislukt.');
        return;
      }
      setDone(true);
      ...
      >
        {!token ? (
          <div className="text-center py-6">
            ...
            <h2 className="text-2xl font-black uppercase tracking-tighter italic mb-3">Ongeldige link</h2>
            <p className="text-sm font-bold text-slate-500 mb-8">
              Deze herstellink is onvolledig. Vraag een nieuwe aan.
            </p>
            <Link
          ...
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center py-6">
            ...
            <h2 className="text-2xl font-black uppercase tracking-tighter italic mb-3">Gelukt!</h2>
            <p className="text-sm font-bold text-slate-500">
              Je wachtwoord is opnieuw ingesteld. Je wordt doorgestuurd naar de inlogpagina...
            </p>
          </motion.div>
        ) : (
          <>
            <h1 className="text-3xl font-black uppercase tracking-tighter italic leading-none mb-2">Nieuw wachtwoord</h1>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-8">
              Kies een nieuw wachtwoord voor je account
            </p>

            <form onSubmit={handleSubmit} className="space-y-4">
              <PasswordField placeholder="NIEUW WACHTWOORD" value={password} onChange={setPassword} />
              <PasswordField placeholder="BEVESTIG WACHTWOORD" value={confirm} onChange={setConfirm} />
